        ...
        public void AddTextOutline(Color color, float size) {
            foreach (Transform textComplexTrans in transform) {
                if (textComplexTrans.GetComponent<Text>() != null) {
                    Outline objectOutline = textComplexTrans.gameObject.AddComponent<Outline>();
                    objectOutline.effectColor = color;
                    objectOutline.effectDistance = new Vector2(size, size);
                }
            }
        }

...
public class LoadingProgressBarAnimation : MonoBehaviour
{
    ...
    private void AnimateLoadingBar()
    {
        ...
        //Set the tweens
        DOTween.To(() => 0, x=> m_LoadingPercentageText.text = $"{x}%", 100, 5f).SetEase(Ease.Linear);
        DOTween.To(() => m_LoadingProgressBar.worldBound.width, x => m_LoadingProgressBar.style.width = x, endWidth, 5f).SetEase(Ease.Linear);
        
        Debug.Log("progress bar width : " + m_LoadingProgressBar.worldBound.width);
        Debug.Log("style width : " + m_LoadingProgressBar.style.width);
    }
}
